...
from app.core.errors import AppError, ExternalServiceError, NotFoundError, ValidationError
        ...
        if document_ids:
            ...
            if missing_ids:
                raise NotFoundError(
                    ...
                    details={"document_ids": [str(document_id) for document_id in missing_ids]},
                )
            not_ready = [document for document in documents if document.status != DocumentStatus.READY]
            if not_ready:
                raise ValidationError(
                    "Selected documents must finish processing before they can be searched.",
                    code="documents_not_ready",
                    details={"document_ids": [str(document.id) for document in not_ready]},
